...
- 今週・今月の合計距離を表示
- 履歴カレンダーと週別距離グラフに反映
- フォームとランニング豆知識の一言アドバイスを日替わり表示
- 距離・時間・平均ペースに応じて一言アドバイスを自動調整
- 正しいフォーム6項目と走る前の8〜10分の準備運動を学べるガイド
...
- Supabaseへログインすると、起動時にクラウドを先に確認して記録を復元
- ログイン後の入力は自動保存され、別端末でも同じ記録を利用可能
- 両方の端末で同時に更新した場合は、どちらを残すか確認してから反映
...
ログイン時は必ずクラウド側を読み取ってから同期方向を決めます。新しい端末の空データで既存のクラウド記録を上書きしません。フォーム動画と動画の登録状態は端末ごとに管理します。
